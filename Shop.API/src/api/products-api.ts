  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      ...
      const [similarProducts] = await connection.query<IProductEntity[]>(
        `SELECT p.*
     FROM products p
     JOIN similar_products sp ON p.product_id = sp.product_title
     WHERE sp.similar_title = ?`,
        [req.params.id]
      );
...
productsRouter.post(
  ...
  async (req: Request<{}, {}, ImagesRemovePayload>, res: Response) => {
    try {
      ...
      const [info] = await connection.query<OkPacket>(IMAGES_DELETE_QUERY, [
        [imagesToRemove],
      ]);
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      ...
      const [rows] = await connection.query<ISimilarProductEntity[]>(
        "SELECT * FROM similar_products where similar_title = ? or product_title = ?",
      ...
      const similarProductsIds = rows.map(
        ({ similar_title, product_title }) => {
          return similar_title === originProduct
            ? product_title
            : similar_title;
        }
      );
...
productsRouter.post(
  ...
  async (req: Request<{}, {}, AddSimilarProductsPayload>, res: Response) => {
    try {
      ...
      await connection.query<OkPacket>(
        "INSERT INTO similar_products (similar_title, product_title) VALUES ?",
        [req.body]
      );
